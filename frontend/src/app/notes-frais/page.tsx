...
export default function NotesFraisPage() {
  ...
  async function loadNotesFrais(currentSocieteId: string) {
    let url = `${API_URL}/societes/${currentSocieteId}/notes-frais`;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await loadNotesFrais(societeId);
    } catch (err: any) {
      setError(err.message || "Erreur inconnue");
    }
  }
        ...
        <header className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-500">
              Gérez vos notes de frais et leur validation.
            </p>
          </div>
          <PeriodFilter
