...
export default function ExpertPrevisionsTresoreriePage() {
  ...
  async function loadPrevisions() {
    if (!societeId) return;
    const res = await fetch(`${API_URL}/societes/${societeId}/previsions-tresorerie?nbMois=${nbMois}`);
    ...
    <div className="min-h-screen bg-zinc-50 px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-0.5 text-sm text-zinc-500">
              Projection sur {nbMois} mois pour cette société (moyennes + factures à encaisser).
            </p>
          </div>
          <select
            value={nbMois}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <section className="rounded-xl bg-white p-5 shadow-md">
            <p className="text-xs font-medium uppercase text-zinc-500">Factures à encaisser (1er mois)</p>
