...
export default function ExpertAlertesPage() {
  ...
  async function loadAlertes() {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/societes/${societeId}/alertes?${params.toString()}`,
      );
      if (!res.ok) {
  ...
  }, [societeId, filtreStatut, filtreType, filtreSeverite]);
              ...
              <div className="flex justify-between items-start">
                ...
                <div className="flex gap-2 ml-4">
                  {alerte.lien && (
                    <button
                      onClick={() => {
                        router.push(`/expert/societes/${societeId}${alerte.lien}`);
