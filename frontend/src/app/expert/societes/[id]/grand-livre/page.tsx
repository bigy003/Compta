...
export default function ExpertGrandLivrePage() {
  ...
  async function loadEcritures() {
    try {
      setLoading(true);
      setError(null);
      let url = `${API_URL}/plan-comptable/societes/${societeId}/ecritures`;
  ...
  useEffect(() => {
    async function init() {
      await Promise.all([loadEcritures(), loadComptes()]);
    }
    if (societeId) {
      init();
    }
        ...
        <header className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-500">
              Consultez toutes les écritures comptables de cette société.
            </p>
          </div>
          ...
          <section className="rounded-xl bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() =>
                    router.push(`/expert/societes/${societeId}/plan-comptable`)
                  }
