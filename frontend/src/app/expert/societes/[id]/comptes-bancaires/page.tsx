...
export default function ExpertComptesBancairesPage() {
    ...
    try {
      ...
      for (const compte of data) {
        const resSolde = await fetch(
          `${API_URL}/societes/${societeId}/comptes-bancaires/${compte.id}/solde`,
        );
        if (resSolde.ok) {
      ...
      <main className="w-full max-w-5xl space-y-6">
        <header>
          ...
          <p className="text-sm text-zinc-500">
            Gérez les comptes bancaires de cette société client.
          </p>
        </header>
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() =>
                            router.push(
                              `/expert/societes/${societeId}/comptes-bancaires/${compte.id}/transactions`,
                            )
                          }
                          className="rounded-lg border border-zinc-200 px-3 py-1.5 text-xs font-medium text-zinc-700 hover:bg-zinc-50"
                        >
