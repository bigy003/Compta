...
export default function ExpertTransactionsPage() {
  const router = useRouter();
  const params = useParams<{ id: string; compteId: string }>();
  ...
  async function loadTransactions(loadAll: boolean = false) {
    if (!societeId) return;
    let url = `${API_URL}/societes/${societeId}/comptes-bancaires/${compteId}/transactions`;
  ...
  async function loadSolde() {
    if (!societeId) return;
    const res = await fetch(
      `${API_URL}/societes/${societeId}/comptes-bancaires/${compteId}/solde`,
    );
    if (res.ok) {
      const data = await res.json();
      setSolde(data);
  ...
  async function loadRecettesNonRapprochees() {
    if (!societeId) return;
    const month = parseInt(selectedMonth);
    const year = parseInt(selectedYear);
    const from = new Date(year, month, 1).toISOString();
    ...
    const res = await fetch(
      `${API_URL}/societes/${societeId}/comptes-bancaires/recettes-non-rapprochees?from=${from}&to=${to}`,
    );
    if (res.ok) {
      const data = await res.json();
  ...
  async function loadDepensesNonRapprochees() {
    if (!societeId) return;
    const month = parseInt(selectedMonth);
    const year = parseInt(selectedYear);
    const from = new Date(year, month, 1).toISOString();
    ...
    const res = await fetch(
      `${API_URL}/societes/${societeId}/comptes-bancaires/depenses-non-rapprochees?from=${from}&to=${to}`,
    );
    if (res.ok) {
      const data = await res.json();
  ...
  useEffect(() => {
    if (societeId) {
      loadTransactions();
      loadSolde();
      loadRecettesNonRapprochees();
      loadDepensesNonRapprochees();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  async function handleImportReleve(file: File | undefined) {
    ...
    try {
      ...
      console.log('[Frontend Expert] Résultat import:', result);
      
      // Afficher une notification selon le cas
      if (result.dejaImporte) {
        console.log('[Frontend Expert] Affichage notification: fichier déjà importé');
        setNotification({
          message: result.message || 'Ce fichier a déjà été importé.',
          type: 'warning',
        });
      } else {
        console.log('[Frontend Expert] Affichage notification: import réussi');
        setNotification({
      ...
      setTimeout(() => {
        console.log('[Frontend Expert] Masquage notification après 5s');
  ...
  async function handleRapprocherAvecRecette(transactionId: string, recetteId: string) {
    ...
    try {
      ...
      await loadTransactions();
      await loadRecettesNonRapprochees();
    } catch (err: any) {
      setError(err.message || "Erreur inconnue");
    }
  }
  ...
  async function handleRapprocherAvecDepense(transactionId: string, depenseId: string) {
    ...
    try {
      ...
      setShowRapprochementModal(null);
      await loadTransactions();
      await loadDepensesNonRapprochees();
    } catch (err: any) {
      setError(err.message || "Erreur inconnue");
    }
  }
