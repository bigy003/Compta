...
export default function StockPage() {
  ...
  async function loadInventaires(id: string) {
    const res = await fetch(`${API_URL}/societes/${id}/stock/inventaires`);
  ...
  async function handleCreateInventaire(e: React.FormEvent) {
    ...
    try {
      ...
      setInvCommentaire("");
      await loadInventaires(societeId);
    } catch (err: unknown) {
      setNotification({ type: "error", message: err instanceof Error ? err.message : "Erreur" });
    }
  }
    ...
    <div className="min-h-screen bg-zinc-50 px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl space-y-6">
        <header>
          ...
          <p className="mt-0.5 text-sm text-zinc-500">Gestion des produits et inventaires physiques.</p>
        </header>
            ...
            {activeTab === "produits" && (
              <div className="space-y-6">
                ...
                {!showFormMouvement && (
                  <button
                    type="button"
                    onClick={() => setShowFormMouvement(true)}
            ...
            {activeTab === "inventaires" && (
              <div className="space-y-6">
                <div className="flex justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => setShowFormInventaire((v) => !v)}
                    className="rounded-lg px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:opacity-90"
                    style={{ backgroundColor: TEAL }}
                  >
                ...
                {inventaires.length === 0 ? (
                  <p className="text-sm text-zinc-500">Aucun inventaire. Créez un inventaire pour enregistrer un comptage physique.</p>
                ) : (
                    ...
                    {inventaires.map((inv) => (
                      <li key={inv.id} className="rounded-lg border border-zinc-200 bg-white p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            type="button"
                            onClick={() => router.push(`/stock/inventaire/${inv.id}`)}
