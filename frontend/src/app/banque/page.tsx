...
interface TransactionBancaire {
  ...
  rapprochementsComptables?: Array<{
    id: string;
    statut: string;
...
export default function BanquePage() {
  ...
  async function initData() {
    try {
      ...
      if (societes.length === 0) {
        setError("Aucune société trouvée. Créez d'abord une société.");
        setLoading(false);
        return;
      }
      const societe = societes[0];
      setSocieteId(societe.id);

      // Charger les comptes bancaires
      const resComptes = await fetch(`${API_URL}/societes/${societe.id}/comptes-bancaires`);
      if (resComptes.ok) {
  ...
  async function chargerTransactions() {
    if (!societeId || !compteSelectionne) return;
    ...
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => router.push("/banque/import")}
      ...
      <div className="mb-4 flex gap-2">
        ...
        <button
          onClick={() => router.push(`/banque/rapprochement?compte=${compteSelectionne}`)}
                ...
                >
                  ...
                  <td className="px-4 py-3 text-center">
                    <button
                      onClick={() =>
                        router.push(
                          `/banque/rapprochement?transaction=${transaction.id}`
                        )
                      }
