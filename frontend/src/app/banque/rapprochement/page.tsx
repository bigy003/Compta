...
function RapprochementContent() {
  ...
  async function initData() {
    try {
      ...
      const resComptes = await fetch(`${API_URL}/societes/${societes[0].id}/comptes-bancaires`);
      if (resComptes.ok) {
        const data = await resComptes.json();
...
export default function BanqueRapprochementPage() {
  return (
